...
public interface BleresRepo extends JpaRepository<Bleres, Integer> {
	...
	@Query( value = " select count(*) as nrSitjeshPerKategori , p.kategoria_produktit as kategoriaProduktit from bleres b inner join produkte p on b.kod_produkti = p.id where data_blerjes_produktit BETWEEN ?1 AND ?2 group by kategoria_produktit ;" ,nativeQuery = true   )
	public List<ShitjeKategoriDTO> getSellsCategory( Date startData , Date endData );    //  ShitjeKategori
